...
pytestmark = pytest.mark.skipif(
    not (torch.cuda.is_available() or (hasattr(torch, "npu") and torch.npu.is_available())),
    reason="UcmYuanrongStore requires a device (CUDA / NPU) for D2H/H2D transfers",
)

logger = init_logger(__name__)

# ---------------------------------------------------------------------------
# Device auto-detection — prefer CUDA, fall back to Ascend NPU
# ---------------------------------------------------------------------------
if torch.cuda.is_available():
    ...
    DEVICE = "cpu"  # will be skipped by pytestmark above
